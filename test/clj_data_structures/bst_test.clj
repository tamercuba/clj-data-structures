...
(defn- build-tree [& values]
  (reduce bst/add (bst/make-tree) values))
...
(deftest make-tree-test
  ...
  (testing "from a collection produces a sorted tree"
    (is (= [1 3 5 7] (bst/to-seq (bst/make-tree [5 3 7 1])))))
...
(deftest add-test
  (testing "first add becomes the root"
    (let [t (bst/add (bst/make-tree) 5)]
  ...
  (testing "add does not mutate — returns a new tree"
    (let [t  (bst/make-tree)
          t1 (bst/add t 5)
          t2 (bst/add t1 3)]
      (is (nil? (:root t)))
  ...
  (testing "throws when value does not satisfy pred"
    (is (thrown? clojure.lang.ExceptionInfo (bst/add (bst/make-tree) "a"))))

  (testing "exception carries the offending value"
    (try
      (bst/add (bst/make-tree) :bad)
...
(deftest delete-test
  (testing "deleted value is no longer a member"
    (let [t (-> (build-tree 5 3 7) (bst/delete 3))]
      (is (false? (bst/member? t 3)))))

  (testing "other values survive deletion"
    (let [t (-> (build-tree 5 3 7 1 4) (bst/delete 3))]
      (are [v] (bst/member? t v)
        5 7 1 4)))

  (testing "delete leaf"
    (let [t (-> (build-tree 5 3) (bst/delete 3))]
      (is (nil? (get-in t [:root :left])))))

  (testing "delete node with one child"
    (let [t (-> (build-tree 5 3 1) (bst/delete 3))]
      (is (bst/member? t 1))
      (is (false? (bst/member? t 3)))))
  ...
  (testing "delete root"
    (let [t (-> (build-tree 5 3 7) (bst/delete 5))]
  ...
  (testing "throws when value does not satisfy pred"
    (is (thrown? clojure.lang.ExceptionInfo
                 (bst/delete (build-tree 5) "x")))))

(deftest to-seq-test
  (testing "empty tree returns nil"
    (is (nil? (bst/to-seq (bst/make-tree)))))

  (testing "single element"
    (is (= [5] (bst/to-seq (build-tree 5)))))

  (testing "returns values in ascending order"
    (is (= [1 3 4 5 7] (bst/to-seq (build-tree 5 3 7 1 4)))))

  (testing "order is independent of insertion order"
    (is (= (bst/to-seq (build-tree 5 3 7 1 4))
           (bst/to-seq (build-tree 1 7 3 4 5)))))

  (testing "in-order after delete is still sorted"
    (let [t (-> (build-tree 5 3 7 1 4) (bst/delete 3))]
      (is (= [1 4 5 7] (bst/to-seq t))))))
...
(deftest min-val-test
  ...
  (testing "min after deleting the minimum"
    (is (= 3 (bst/min-val (-> (build-tree 5 3 7 1) (bst/delete 1)))))))
...
(deftest max-val-test
  ...
  (testing "max after deleting the maximum"
    (is (= 5 (bst/max-val (-> (build-tree 5 3 7) (bst/delete 7)))))))
